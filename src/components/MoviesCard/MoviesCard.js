...
import { BASE_MOVIES_URL, MINUTES_IN_HOUR } from '../../utils/constants';
...
function MoviesCard(props) {
  ...
  const MOVIE_HOURS = (parseInt(cardDuration / MINUTES_IN_HOUR));
  const MOVIE_MINUTES = cardDuration % MINUTES_IN_HOUR;

  useEffect(() => {
    isMovies && savedMovies.map((movie) => {
  ...
  useEffect(() => {
    const savedMoviesMovieId = isMovies && savedMovies.map((movie) => {
      return movie.movieId
    })
    if (isMovies && savedMoviesMovieId.includes(cardId)) {
      setIsLiked(true)
    }
  }, [savedMovies])
